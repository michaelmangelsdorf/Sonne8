...
uint8_t ser_in() { return 0;}
uint8_t par_in() { return 0;}
void ser_out( uint8_t source) {}
...
void exec_SIGNAL( uint8_t instr)
{
	...
	if (dest) { // Transfer
		...
		switch (source) {
		case 0: break; /* NOP */
		case 1: serial_bit_in(); break; /* SCI */
		case 2: serial_bit_out(); break; /* SCO */
