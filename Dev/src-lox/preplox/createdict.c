/*
    Create an initial symbol table file (listsrc.asm) in LOX format.
    Paste this fragment into asciimem.txt manually at 0x8000.
    asciimem.txt is created by strlgen.c.
...
int createDict(struct myth_vm*);
void injectDict( struct myth_vm*, int size);
...
/* Compile a dictionary of String-to-Opcode mappings
   within the controller's memory starting at 0x8000
   Format:
    List of: <0-terminated str><opcode>
    List terminated by three consecutive zeros.
   Returns list size
 */
int
createDict( struct myth_vm *vm)
{
        int elem, i, size;
        uchar *addr;
        char ch;
        ...
        elem = 0;
        size = 0;
        addr = &vm->pagebyte[0x80][0];
        while( strcmp( strlits[elem].str, "HALT") != 0){

                i=0;
                do{     ch = strlits[elem].str[i++];
                        *addr++ = ch;
                        size++;
                }
                while( ch != 0);
                *addr++ = strlits[elem].val;
                elem++;
                size++;
        }
        *addr++ = 0;
...
/* Write the dictionary portion of a fresh NETTLE source file
 */
void
injectDict( struct myth_vm * vm, int size)
{
        char *fname = "listsrc.asm";
        int fdesc;
        uchar *ptr;
        ...
        ptr = &vm->pagebyte[0x80][0];
        if(fdesc != -1){
                int addr = 0x8000;
                for(;;){
                        if (*ptr == 0) break;
                        while(*ptr != 0){
                                if (addr%256 == 0) {fprint(fdesc,"         @\n");}  
                                fprint(fdesc, "    %.04X@ '%c'        ;", addr, *ptr);
                                addr++;
                                ptr++;
                                fprint(fdesc,"\n");
                                if (addr%256 == 0) {
                        ...
                        if (addr%256 == 0) {fprint(fdesc,"         @\n");}  
                        fprint(fdesc, "    %.04X@ %.02Xh        ;", addr, *ptr);
                        fprint(fdesc,"\n");
                        addr++;
                        ptr++;
                        if (addr%256 == 0) {
