...
/* Meighty - a machine code assembler for the Sonne micro-controller
...
#define LHS_SIG 0
#define LHS_L   1  // Literal
#define LHS_G   2  // Global
#define LHS_M   3  // Memory
#define LHS_R   4  // Row
#define LHS_F   5  // Function
#define LHS_S   6  // Serial
#define LHS_P   7  // Parallel

#define RHS_Q   0  // Quick
#define RHS_A   1  // At
#define RHS_G   2  // Global
#define RHS_M   3  // Memory
#define RHS_R   4  // Row
#define RHS_C   5  // Call
#define RHS_F   6  // Function
#define RHS_X   7  // Pending pointer
#define RHS_Y   8  // Pending branch
#define RHS_B   9  // Branch
#define RHS_T   10 // Then
#define RHS_E   11 // Else
#define RHS_U   12 // Internal
#define RHS_V   13 // Utility
#define RHS_S   14 // Serial
#define RHS_P   15 // Parallel
...
void gen_opcodes()
{
    ...
    for (int i=0; i<8; i++){
        for (int j=0; j<16; j++)
        {
            ...
            if (i==LHS_L && j==RHS_M) {
                strcpy( mnemo_decoder[i*16+j], "RET" );
                continue;
            }
            if (i==LHS_M && j==RHS_M) {
                strcpy( mnemo_decoder[i*16+j], "PULL" );
                continue;
            }
            if (i==LHS_G && j==RHS_G) {
                strcpy( mnemo_decoder[i*16+j], "CLIP" );
                continue;
            }
            if (i==LHS_R && j==RHS_R) {
                strcpy( mnemo_decoder[i*16+j], "LID" );
                continue;
            }
            if (i==LHS_SIG) { /* Signals */
                strcat( mnemo_decoder[i*16+j], rhs_S[j]);
                //printf("%s\n", mnemo_decoder[i*16+j+128]);
            }
            else { /* Transfers */ 
                strcpy( mnemo_decoder[i*16+j], lhs[i] );
//                if (j==RHS_B || j==RHS_BR || j==RHS_BRZ || j==RHS_BT) {
//                    strcat( mnemo_decoder[i*16+j], "'");
//                }
                strcat( mnemo_decoder[i*16+j], rhs_T[j]);
            }
        }
    }

    // TRAP instructions (64 opcodes, 128-191)
    for (int i=128; i<192; i++) strcpy(mnemo_decoder[i], "");
        ...
        for (int j=0; j<2; j++)
            for (int k=0; k<8; k++)
                for (int l=0; l<2; l++)
                {
                    ...
                    strcpy( mnemo_decoder[i*32 + j*16 + l*8 + k + 192], i==0 ? "A" : "Q");
...
uint8_t ROM_mem[128*128]; /* 128 (ROM only) frames * 128 bytes = 32k */
unsigned ROM_srcLine[128*128]; /* Which line number generated the output byte */
...
/* Clear structures */
    ...
    void clear()
    {
        for (int i=0; i<(128*128); i++) ROM_mem[i] = 0;

        for (int i=0; i<128; i++) 
            strcpy(frame_mnemo[i],"");
...
/* Validate label references */
    ...
    int find_backref(char* refstr)
    {
        ...
        unsigned here;
            ...
            for (int i=here; i>=0; i--)
                if (!strcmp(refstr, frame_mnemo[i])) return i;
        return -1;
    }
    ...
    int find_mcref(char* refstr)
    {
        //printf("FWD REFSTR=%s\n", refstr);
        unsigned here;
        for (int i=0; i<128; i++) if (!strcmp(refstr, frame_mnemo[i])) return i;
        return -1;
    }
        ...
        while (skipspace() == 0)  /* Skip leading white space */
        {
            ...
            if (!handled && strlen(wordbuf)>2) {
                ...
                if (wordbuf[endpos]=='@' && wordbuf[0] != '"') {
                    ...
                    defmap[defmap_topindex].val = objcursor + 1;
                    defmap[defmap_topindex].frame = objframe;
                    defmap_topindex++;
                    handled = 1;
                }
            }
            ...
            if (wordbuf[0]=='@') {
                ...
                { /* Check if it's a frame label definition @LABEL */
                        ...
                        strcpy(frame_mnemo[objframe], wordbuf+1);
             ...
             */
            if (endframe || !strcmp(wordbuf,"LID") || !strcmp(wordbuf,"RET") )
            {
                    if (objframe+1 > 255){
                ...
                if (endframe) store(LHS_R * 16 + LHS_R); // LID opcode
...
{
	printf("Meighty assembler, rev.22-Oct\n");
	printf("(See http://ok-schalter.de/sonne for details)\n\n");

	src = readsrcf("bootstrap.asm");
    if (!src) {
        printf("This program requires an assembly source code\n");
        printf("input file named 'bootstrap.asm', which was not found...\n\n");
    ...
    for (pass=1; pass<3; pass++) {
        ...
        for (int i=0; i<128; i++) frame_lid[i] = 0;
        for (int i=0; i<128; i++) ROM_srcLine[i] = 0;
        ...
        for (int i=0; i<objframe; i++)
            if (frame_mnemo_export[i]) {
                 sprintf(tempbuf, "%s %c", frame_mnemo[i], i);
                for (int j=0; j<strlen(frame_mnemo[i])+2; j++) symtab[symtab_index++] = tempbuf[j];
            }
...
//        }
            ...
            /* TODO:  Write tablecode absolute pointers first */
            /* Include number symbols in symbol table! */
    ...
    FILE* f = fopen("debug.obj","w");
    fwrite( ROM_mem, 1, 128*128, f);
    fclose(f);
    ...
    fprintf(f, "\nMcBastard assembly output (pass 2)\n");
