...
"G0A", "G0B", "G0R", "G0W", "L0A", "L0B", "L0R", "L0W", 
"AG0", "BG0", "RG0", "WG0", "AL0", "BL0", "RL0", "WL0", 
"G1A", "G1B", "G1R", "G1W", "L1A", "L1B", "L1R", "L1W", 
"AG1", "BG1", "RG1", "WG1", "AL1", "BL1", "RL1", "WL1", 
"G2A", "G2B", "G2R", "G2W", "L2A", "L2B", "L2R", "L2W", 
"AG2", "BG2", "RG2", "WG2", "AL2", "BL2", "RL2", "WL2", 
"G3A", "G3B", "G3R", "G3W", "L3A", "L3B", "L3R", "L3W", 
"AG3", "BG3", "RG3", "WG3", "AL3", "BL3", "RL3", "WL3", 
"NN", "RET", "NR", "NW", "ND", "NL", "NS", "NP", 
"NE", "NJ", "NI", "NT", "NF", "NC", "NA", "NB", 
"MN", "REA", "MR", "MW", "MD", "ML", "MS", "MP", 
"ME", "MJ", "MI", "MT", "MF", "MC", "MA", "MB", 
"RN", "RM", "REB", "RW", "RD", "RL", "RS", "RP", 
"RE", "RJ", "RI", "RT", "RF", "RC", "RA", "RB", 
"WN", "WM", "WR", "RER", "WD", "WL", "WS", "WP", 
"WE", "WJ", "WI", "WT", "WF", "ITC", "WA", "WB", 
"DN", "DM", "DR", "DW", "_", "DL", "DS", "DP", 
"DE", "DJ", "DI", "DT", "DF", "NEX", "DA", "DB", 
"LN", "LM", "LR", "LW", "LD", "_", "LS", "LP", 
"LE", "LJ", "LI", "LT", "LF", "LC", "LA", "LB", 
"SN", "SM", "SR", "SW", "SD", "SL", "_", "SP", 
"SE", "SJ", "SI", "ST", "SF", "SC", "SA", "SB", 
"PN", "PM", "PR", "PW", "PD", "PL", "PS", "_", 
"PE", "PJ", "PI", "PT", "PF", "PC", "PA", "PB"
};
...
void
myth_set_R( Myth_vm *vm, uint8_t val)
{
    vm->reg_R_cache = vm->reg_R;
    vm->reg_R = val;
}
...
{
    vm->reg_B_cache = vm->reg_B;
    vm->reg_B = val;
    vm->xMx = vm->reg_B;
}

void
myth_set_W( Myth_vm *vm, uint8_t val)
{
    vm->reg_W = val;
    vm->xMx = vm->reg_W;
}
...
myth_ALU( Myth_vm *vm, uint8_t func)
{
    switch(func & 15)
    {
        case ALU_IDA: myth_set_R(vm, vm->reg_A); break;
        case ALU_IDB: myth_set_R(vm, vm->reg_B); break;
        case ALU_OCA: myth_set_R(vm, ~vm->reg_A); break;
        case ALU_OCB: myth_set_R(vm, ~vm->reg_B); break;
        case ALU_SLA: myth_set_R(vm, vm->reg_A << 1); break;
        case ALU_SLB: myth_set_R(vm, vm->reg_B << 1); break;
        case ALU_SRA: myth_set_R(vm, vm->reg_A >> 1); break;
        case ALU_SRB: myth_set_R(vm, vm->reg_B >> 1); break;
        case ALU_AND: myth_set_R(vm, vm->reg_A & vm->reg_B); break;
        case ALU_IOR: myth_set_R(vm, vm->reg_A | vm->reg_B); break;
        case ALU_EOR: myth_set_R(vm, vm->reg_A ^ vm->reg_B); break;
        case ALU_ADD: myth_set_R(vm, vm->reg_A + vm->reg_B); break;
        case ALU_CAR: myth_set_R(vm, (int) vm->reg_A + (int) vm->reg_B > 255 ? 1 : 0); break;
        case ALU_ALB: myth_set_R(vm, (vm->reg_A < vm->reg_B) ? 255 : 0); break;
        case ALU_AEB: myth_set_R(vm, (vm->reg_A == vm->reg_B) ? 255 : 0); break;
        case ALU_AGB: myth_set_R(vm, (vm->reg_A > vm->reg_B) ? 255 : 0); break;
    }
}
...
myth_write_reg( Myth_vm *vm, uint8_t selector, uint8_t val)
{
    ...
    switch (selector & 15)
    {
        ...
        case RHS_M:
            ...
            break;
        case RHS_R: myth_set_R(vm, val); break;
        case RHS_W: myth_set_W(vm, val); break;
        case RHS_D: vm->reg_D = val; break;
        case RHS_L: vm->reg_L = val; break;
        case RHS_S: vm->reg_SOR = val; break;
        case RHS_P: myth_parallel_out(vm, val); break;
        case RHS_E: vm->reg_E = val; break;
        case RHS_J: myth_jump(vm, val); break;
        case RHS_I: vm->reg_I--; if (vm->reg_I) myth_jump(vm, val); break;
        case RHS_T: if (vm->reg_R) myth_jump(vm, val); break;
        case RHS_F: if (!vm->reg_R) myth_jump(vm, val); break;
...
myth_read_reg( Myth_vm *vm, uint8_t selector)
{
    ...
    switch (selector & 7)
    {
        ...
        case LHS_W: return vm->reg_W;
        case LHS_D: return vm->reg_D;
        case LHS_L: return vm->reg_L;
...
void
myth_PAIR( Myth_vm *vm, uint8_t dst, uint8_t src)
{
    src &= 3;
    dst &= 15;
    ...
    else if ((src==LHS_W) && (dst==RHS_C)) { // Scrounge
    }
    else if ((src==LHS_D) && (dst==RHS_C)) { // Scrounge
    }
    else if (src!=0 && src==dst)
    switch (src) {
        case SCR_REA: myth_set_A(vm, vm->reg_A_cache); break;
        case SCR_REB: myth_set_B(vm, vm->reg_B_cache); break;
        case SCR_RER: myth_set_R(vm, vm->reg_R_cache); break;
        default:;
    }
...
myth_GETPUT( Myth_vm *vm, uint8_t bits)
{
    ...
    switch(guide) {
        ...
        case 2: if (PUTBIT) vm->ram[addr] = vm->reg_R;
                else myth_set_R(vm, vm->ram[addr]);
                break;

        case 3: if (PUTBIT) vm->ram[addr] = vm->reg_W;
                else myth_set_W(vm, vm->ram[addr]);
                break;
    }
}
...
{
    uint8_t src = opcode & 15;
    uint8_t dst = (opcode >> 4) & 3;
    uint8_t addend;
    ...
    if (opcode & 16) { myth_ALU(vm, src); return; }
    if (opcode & 8) {
        switch(src&7) {
            ...
            default: addend = (uint8_t) (0xF8 | (src&7));
        }
        myth_set_R( vm, vm->reg_R + addend);
        return;
    }
    else myth_SYS(vm, opcode & 7);
}
    ...
    if (myth_rdimg(&vm, fname)){
       ...
       exit(-1);
        ...
        for (i=0; i<MAX_CYCLES; i++) {
            ...
            //printf("A=%02X B=%02X R=%02X C=%02X PC=%02X D=%02X W=%02X L=%02X I=%02X\n",
            //    vm.reg_A, vm.reg_B, vm.reg_R, vm.reg_C, vm.reg_PC, vm.reg_D,
            //    vm.reg_W, vm.reg_L, vm.reg_I);

            if (quitf) break;
        }
    }
    printf("Ax%02X Bx%02X Rx%02X\n", vm.reg_A, vm.reg_B, vm.reg_R);
