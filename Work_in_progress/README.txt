...
This is a CPU design for an 8-bit microcontroller. Addresses are formed by concatenating an 8-bit "page" address prefix and a 7-bit address "offset" byte (of which the MSB selects a paging register), see below. The resulting memory layout consists of 256 pages of 128 bytes each, thus 32 kilobytes of memory.
...
(Starting from the most significant bit of the instruction opcode)
	...
	Result adjustment instructions

	If bits 7-3 of the opcode are 0000_1, the opcode represents an adjustment instruction. The instructions add or subtract a small integer number from R. The mnemonic form is: "P" + decimal value for positive values, and "M" + decimal value for negative values.
...
R-Adjustment Mnemonics

P2 is the mnemonic for the opcode that adds 1 to R. M1 is the mnemonic for the opcode that subtracts 1 from R.
