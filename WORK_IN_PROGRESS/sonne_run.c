...
#define LHS_H     2  /* BANK REG Half */

/* The H register is a bank register.
   ...
   The CPU assumes that the H register contains the bank
   ...
   number to return to into the H register.
   During RET, the original bank number is restored from H.
   ...
   of 0-127 (128 bytes from 0-7Fh), called Half segment,
   the implied memory bank (high-order byte) is in H.

   Example: "nh 10 na 3 mb" sets the bank number for
   ...
   the range 0-127 (the Half segment),
...
#define RHS_H     2  /* BANK REG Exit - explained above */
...
/* Three COPY PAIRS are "scrounged" and
   ...
   HH is executed as CLIPA
   GG is executed as CLIPB
...
/* The following are the CPU registers and memory. */


/* aacc and bacc are the accumulator registers.
   The one most recently written to is called W,
   the WORKING accumulator.
   It holds the implied address offset for memory
   operations.
   The *wptr pointer points to the most recently
   written to accumulator register.
   Whenever an accumulator register is written to,
   wclip receives a copy of the previous value.
   The CLIPA/B instruction copies that value back
   into A or B.
*/

uint8_t wclip, aacc, bacc, *wptr,

/* zflag reflects the zero status of the R register.
   When R is zero, it is set to one, else zero.
   alu__offs contains an offset value (set via OFFS
   instructions) which is implicitly added to R.
*/

		zflag, alu_offs,

/* pc_offs is the low-order byte of the
   program counter.
   rreg (R register) is overwritten by a copy
   of it during calls/TRAPS.
   pc_bank is the high-order byte of the
   program counter.
   hreg (H register) is overwritten by a copy
   of it during calls/TRAPS.

   Each ALU function instruction places its result
   in rreg.

   hreg is the bank register for the HALF segment
   (offsets 0-127, 128 bytes).
  */

		pc_offs, rreg,
		pc_bank, hreg,

/* greg is the bank register for the GLOBAL segment
   (offsets 128-191, 64 bytes).
   lreg is the bank register for the LOCAL segment
   (offsets 192-255, 64 bytes).
  */

		greg, lreg,

/* The bit pattern in dreg selects/deselects
   communication devices.
   By convention, the lower nybble and the high
   order nybble work independently, for example
   in encoding an active transmitter and receiver,
   respectively.
  */

		dreg,

		serial_in_byte,	serial_out_byte, sclock,
		parallel_bus_in, parallel_bus_out,  tristate,
...
uint8_t calc_zflag(){ zflag = (get_r() == 0) ? 1:0; return zflag; }
...
/* Compute the effective address for
   ...
   The offset is contained in the WORKING
   accumulator register (pointed to by *wptr).
   The offset determines, which bank register
   is used.
   NOTE: instruction fetch or literal fetch
   always use pc_bank!
*/
...
void write_reg( uint8_t dest, uint8_t source)
{
	...
	case RHS_R: set_r(source); break;
	case RHS_D: dreg = source; break;
	case RHS_J: pc_offs = source; break;
	case RHS_T: if (!calc_zflag()) pc_offs = source; break;
	case RHS_F: if (calc_zflag())  pc_offs = source; break;
...
void shift_in()
{
	...
	printf("A=%02X(%d), B=%02X(%d) z=%02X CARRY=%d; f=%02X\n",
	 aacc, aacc, bacc, bacc, zflag, (int) aacc + (int) bacc > 255 ? 1 : 0, get_r());
}
...
void exec_PAIR( uint8_t instr)
{
	...
	if (dest==RHS_SIG) {
		...
		case 6: set_r(memory[lreg*256+0xC4]); lreg++; break; /* LEAVE */
		...
		else if ((source == LHS_H) && (dest == RHS_H)) {aacc = wclip;} // Scrounge CLIPA
		else if ((source == LHS_H) && (dest == RHS_H)) {bacc = wclip;} // Scrounge CLIPA
		else write_reg( dest, read_reg( source));
	}
}
...
void exec_ALU( uint8_t instr)
{
	...
	if (alu_opcode == 3) /* ALU function */
	{
		switch(instr & 15){
		case 0: set_r(aacc); break; // IDA
		case 1: set_r(bacc); break; // IDB
		case 2: set_r(~aacc); break; // OCA
		case 3: set_r(~bacc); break; // OCB
		case 4: set_r(aacc << 1); break; // SLA
		case 5: set_r(bacc << 1); break; // SLB
		case 6: set_r(aacc >> 1); break; // SRA
		case 7: set_r(bacc >> 1); break; // SRB
		case 8: set_r(aacc & bacc); break; // AND
		case 9: set_r(aacc | bacc); break; // IOR
		case 10: set_r(aacc ^ bacc); break; // EOR
		case 11: set_r(aacc + bacc); break; // ADD
		case 12: set_r( (int) aacc + (int) bacc > 255 ? 1 : 0 ); break; // CARRY
		case 13: set_r( (aacc < bacc) ? 255 : 0 ); break; // ALB
		case 14: set_r( (aacc == bacc) ? 255 : 0 ); break; // AEB
		case 15: set_r( (aacc > bacc) ? 255 : 0 ); break; // AGB
		}
	}
	else { /* GETPUT instruction */
		switch(alu_opcode) {
		...
		if (instr & 8) { // PUT
			 if (loc==&rreg) memory[addr] = get_r();
			 else memory[addr] = *loc;
		}
		else {// GET
			if (loc == &rreg) set_r(memory[addr]);
			else *loc = memory[addr];
		}
	}
}
