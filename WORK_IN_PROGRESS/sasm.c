    ...
    // APPLY instructions (opcodes 0-127)
        for (int j=0; j<8; j++)
        {
            for (int i=0; i<16; i++)
           {
            ...
            else if (i==RHS_R && j==LHS_R) {
                strcpy( mnemo_decoder[j*16+i], "CALL" );
            }
            else if (i==RHS_G && j==LHS_G) {
                strcpy( mnemo_decoder[j*16+i], "EXIT" );
            }
            else if (i==RHS_SIG) { /* Signals */
                strcpy( mnemo_decoder[j*16+i], lhs_S[j]);
                //printf("%s %d %d\n", mnemo_decoder[j*16+i], j*16, i);
            }
            else if (i==RHS_INC) { /* ALU increment */
                strcpy( mnemo_decoder[j*16+i], alu_inc[j]);
            }
            else if (i==RHS_DEC) { /* ALU decrement */
                strcpy( mnemo_decoder[j*16+i], alu_dec[j]);
            }
            else { /* Transfers */ 
            ...
            for (int k=0; k<4; k++)
                for (int l=0; l<2; l++)
                {
                    ...
                    if (i==3) {
                        ...
                        strcpy( mnemo_decoder[i*16 + j*8 + l*4 + k + 192], j==0 ? "G" : "L");
                       ...
                       switch (i) {
                        ...
                        case 2: strcat( mnemo_decoder[i*16 + j*8 + l*4 + k + 192], "f"); break;
                        }
                    }
                    else { // PUT
                       switch (i) {
                        ...
                        case 2: strcpy( mnemo_decoder[i*16 + j*8 + l*4 + k + 192], "f"); break;
                        }
                        strcat( mnemo_decoder[i*16 + j*8 + l*4 + k + 192], j==0 ? "G" : "L");
                        strcat( mnemo_decoder[i*16 + j*8 + l*4 + k + 192], numstr);
                    }
                }
