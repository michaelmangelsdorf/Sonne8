...
void gen_opcodes()
{
    ...
    // GETPUT instructions (64 opcodes, 0-63)
    for (int i=0; i<4; i++)
        for (int j=0; j<2; j++)
            for (int l=0; l<2; l++)
                for (int k=0; k<4; k++)
                {
                    ...
                    if (i==0) {
                         // Kicks
                         strcpy( mnemo_decoder[i*16 + j*8 + l*4 + k], kicks[j*8 + l*4 + k]);
                         
                    }
                    else if (j==0) { // GET
                        strcpy( mnemo_decoder[i*16 + j*8 + l*4 + k], l==0 ? "g" : "l");
                        strcat( mnemo_decoder[i*16 + j*8 + l*4 + k], numstr);
                       switch (i) {
                        case 1: strcat( mnemo_decoder[i*16 + j*8 + l*4 + k], "A"); break;
                        case 2: strcat( mnemo_decoder[i*16 + j*8 + l*4 + k], "B"); break;
                        case 3: strcat( mnemo_decoder[i*16 + j*8 + l*4 + k], "R"); break;
                        }
                    }
                    else { // PUT
                       switch (i) {
                        case 1: strcpy( mnemo_decoder[i*16 + j*8 + l*4 + k], "A"); break;
                        case 2: strcpy( mnemo_decoder[i*16 + j*8 + l*4 + k], "B"); break;
                        case 3: strcpy( mnemo_decoder[i*16 + j*8 + l*4 + k], "R"); break;
                        }
                        strcat( mnemo_decoder[i*16 + j*8 + l*4 + k], l==0 ? "g" : "l");
                        strcat( mnemo_decoder[i*16 + j*8 + l*4 + k], numstr);
                    }
                }

    // TRAP instructions (32 opcodes, 64-95)
    for (int i=64; i<96; i++) {
        sprintf(mnemo_decoder[i], "*%d", i-64);
    }

    // Offset instructions (16 opcodes, 96-111)
    for (int i=96; i<112; i++) {
        strcpy( mnemo_decoder[i], alu_offs[i-96]);
    }

    // ALU instructions (16 opcodes, 112-127)
    for (int i=112; i<128; i++) {
        strcpy( mnemo_decoder[i], aluop[i-112]);
    }
    ...
    for (int j=0; j<8; j++)
    {
            for (int i=0; i<16; i++)
           {
            ...
            if (i==RHS_M && j==LHS_N) {
                strcpy( mnemo_decoder[j*16+i + 128], "RET" );
                //printf("%s %d %d\n", mnemo_decoder[j*16+i], j*16, i);
            }
            else if (i==RHS_M && j==LHS_M) {
                strcpy( mnemo_decoder[j*16+i + 128], "LID" );
            }
            else { /* Transfers */ 
        ...
        while (skipspace() == 0)  /* Skip leading white space */
        {
            ...
            if (!handled)
            {
                ...
                if (found) {
                    ...
                    //printf("opcode = %02X\n", opc_colon);
                    if (!opc_colon) { printf("Invalid : expression %s\n", wordbuf); exit(1); }
                    store(opc_colon);
