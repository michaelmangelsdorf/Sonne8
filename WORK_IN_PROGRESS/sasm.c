    ...
    // APPLY instructions (opcodes 0-127)
        for (int j=0; j<8; j++)
        {
            for (int i=0; i<16; i++)
           {
            ...
            else if (i==RHS_SIG && j!=LHS_ALU) { /* Signals */
                strcpy( mnemo_decoder[j*16+i], lhs_S[j]);
                //printf("%s %d %d\n", mnemo_decoder[j*16+i], j*16, i);
            }
            else if (j==LHS_ALU) { /* ALU opcodes */   
                strcpy( mnemo_decoder[j*16+i], aluop[i]);
            }
            else if (i==RHS_INC && j!=LHS_ALU) { /* ALU increment */
                strcpy( mnemo_decoder[j*16+i], aluinc[j]);
            }
            else if (i==RHS_DEC && j!=LHS_ALU) { /* ALU increment */
                strcpy( mnemo_decoder[j*16+i], aludec[j]);
            }
            else { /* Transfers */ 
